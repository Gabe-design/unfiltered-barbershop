...
const VIP_COLORS: Record<string, string> = {
  bronze: "bg-amber-600", silver: "bg-zinc-400", gold: "bg-yellow-400", platinum: "bg-purple-400",
};
  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <StatCard icon={Calendar} label="Total Bookings" value={overview.totalBookings} sub="All time" color="bg-indigo-500/10 text-indigo-400" />
        ...
        <div className="bg-[#111111] border border-zinc-800 rounded-xl p-5">
          ...
          <div className="space-y-2">
            {[
              ...
              { label: "New Customers", value: thisMonth.newCustomers, color: "text-indigo-400" },
            ].map((r) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4">
        <Panel title="Bookings by Source" icon={TrendingUp}>
          <div className="space-y-2.5">
            {Object.entries(bySource).filter(([, v]) => v > 0).sort(([, a], [, b]) => b - a).map(([src, count]) => (
              <BarRow key={src} label={SOURCE_LABELS[src] ?? src} value={count} max={maxSource} color="bg-indigo-500" />
            ))}
        ...
        <Panel title="Review Requests" icon={Star}>
          <div className="space-y-3">
            {[
              { label: "Sent", value: reviewStats.sent, color: "text-indigo-400" },
        ...
        <Panel title="Abandoned & Referrals" icon={UserX}>
          <div className="space-y-3">
            ...
            {[
              { label: "Total", value: abandonedStats.total, color: "text-indigo-400" },
            ...
            {[
              { label: "Active codes", value: referralStats.totalCodes, color: "text-indigo-400" },
