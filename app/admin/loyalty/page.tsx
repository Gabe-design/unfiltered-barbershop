...
const VIP_CONFIG: Record<VipStatus, { label: string; classes: string; dot: string }> = {
  ...
  PLATINUM: { label: "Platinum", classes: "bg-purple-900/40 text-purple-400", dot: "bg-purple-400" },
};
    ...
    <div className="p-6 space-y-5 max-w-7xl mx-auto">
      ...
      <div className="flex gap-1 flex-wrap">
        <button
          ...
          className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${filter === "all" ? "bg-indigo-500/10 text-indigo-400 border border-indigo-500/20" : "text-zinc-500 hover:text-white border border-transparent"}`}
        >
          All
        </button>
        {VIP_LEVELS.map((level) => {
          ...
          return (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${filter === level ? "bg-indigo-500/10 text-indigo-400 border border-indigo-500/20" : "text-zinc-500 hover:text-white border border-transparent"}`}
            >
              {cfg.label}
            </button>
          );
                    ...
                    <tr key={p.id} className="border-b border-zinc-800/50 hover:bg-zinc-800/20 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <select
                          ...
                          className="bg-zinc-900 border border-zinc-700 rounded-lg px-2 py-1 text-white text-xs focus:outline-none focus:border-indigo-500"
                        >
                          {VIP_LEVELS.map((l) => (
